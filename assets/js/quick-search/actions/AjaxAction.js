...
export default class AjaxAction extends Action {
	...
	this.executeSignal = this.executeController.signal;
		...
		let result = this.wpAjax(this.action, {}, this.executeSignal).then(result => {
			console.log(result);
			this.sendSuccessNotification(`[${result.status}] Response from ${this.label}: <br> ${result.data}`);
		}).catch(result => {
			console.log(result);
			this.sendErrorNotification(`[${result.status}] Response from ${this.label}: <br>${result.statusText}`);
		});
	}
